...
Only the *password* half of `tsh login` is automated. The second factor stays a
human action, whichever method `TELEPORT_MFA_MODE` pins (browser by default
here; `platform` for Touch ID, `otp`, `cross-platform`).
...
`exit=0` means headless works. The plist pins `TLOGIN_MFA_MODE` (browser here,
matching `TELEPORT_MFA_MODE` in `.zshrc`) so tsh doesn't also sit waiting on an
OTP code nobody will type.
